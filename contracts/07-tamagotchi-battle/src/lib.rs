...
extern fn init() {
    ...
    let tamagotchi_battle = Battle {
        ...
        shields_data: BTreeMap::from([(SHIELD_ID, SHIELD)]),
        ..Default::default()
    };
...
#[gstd::async_main]
async fn main() {
    let action: BattleAction = msg::load().expect("Unable to decode `FactoryAction`");
    let tmg_battle = unsafe { TAMAGOTCHI_BATTLE.get_or_insert(Default::default()) };
    ...
    match action {
        BattleAction::Register {
            ...
            tmg_battle
                .initiate_registration(&tamagotchi_id, attributes)
                .await;
        }
        ...
        BattleAction::SendNewAttributesToNextRound { new_attributes } => {
